...
run_lmem_functional <- function(data_df, meta_df, ref_group, column_name, feature_column_name = "Feature", offset=1e-9) {
  library(dplyr)
  library(tidyr)
  library(purrr)
  library(lme4)
  ...
  # Normalize data
  data_df <- normalize_data(data_df, offset = offset)
  ...
  # Reshape and join metadata
  data <- data_df %>%
    rownames_to_column(var = feature_column_name) %>%
    pivot_longer(-all_of(feature_column_name), names_to = "Sample_ID", values_to = "value") %>%
    left_join(meta_df %>% select(all_of(column_name), Cohort, Sample_ID), by = "Sample_ID")
  ...
  for (i in seq_along(Features)) {
    ...
    pr.n <- mean(x.neg$value >= 1e-08)
    pr.p <- mean(x.pos$value >= 1e-08)
    pr.shift[[i]] <- c(pr.p - pr.n, pr.n, pr.p)
  }
...
plot_volcano <- function(plot_df, xBreaks, fdr_thresh = 0.05, man_y_breaks = NULL, nudge_y = NULL, min_segment_length = NULL,
  ...
  if (is.null(max.overlaps)) max.overlaps <- 10
  if (is.null(min_segment_length)) min_segment_length <- 0.1
  if (is.null(nudge_y)) nudge_y <- 3
  
  # Dynamically construct label column using feature_column_name
  plot_df <- plot_df %>%
    mutate(
      ...
      font = ifelse(fdr_sig, "bold.italic", "italic"),
      ...
      lab = case_when(
        P.adj < fdr_thresh ~ .data[[feature_column_name]],
        TRUE ~ ""
      ),
      prev_size = case_when(
        group_prev >= 0.75 ~ 5,
  ...
  sample_sizes <- plot_df %>%
    ...
    mutate(group = gsub("^n\\.", "", group))
  
  # Compute the q-value threshold (optional)
  ...
  pt <- plot_df %>%
    ...
    geom_point(
      aes(size = prev_size, fill = enriched_in),
      shape = 21, color = "black", alpha = 0.8
    ) +
    scale_size(
      range = c(1.5, 6),
      breaks = c(1.5, 2, 3, 5),
      labels = c("0.10", "0.25", "0.50", "0.75"),
    ...
    labs(
      fill = paste0("P adj. < ", fdr_thresh),
      size = "Prevalence\n"
    ) +
    guides(
      color = "none",
      size = guide_legend(order = 1, reverse = TRUE, title = "Prevalence"),
      fill = guide_legend(order = 2, override.aes = list(size = 3.5))
    ) +
    scale_fill_manual(
      values = color_vector,
      labels = enriched_in_labels
    ) +
...
plot_roc_siamcat_models_with_fobt <- function(models, labels, colours, linetypes, trained_on = NULL, alpha = NULL, FOBT_tpr = NULL) {
  ...
  for (i in seq_along(models)) {
    ...
    roc_data$model <- labels[i]  # Explicitly assign model names
    all_roc_data <- rbind(all_roc_data, roc_data)
    
    auc_list[[i]] <- paste0(labels[i], " (AUC = ", round(auc, 3), ", Train N = ", mean_train_size, ", Test N = ", mean_test_size, ")")
  }
  ...
  all_roc_data <- all_roc_data %>%
    mutate(Cohort = str_split_fixed(model, ' ', 2)[,1]) %>% 
    left_join(FOBT_tpr %>% rename(TPR.FOBT=TPR, FPR.FOBT=FPR), by='Cohort') %>% 
    mutate(Trained_on = ifelse(Cohort == 'BaxterNT_2016', 'FIT', 'FOBT'))
  
  unique_labels <- all_roc_data %>%
    distinct(FPR.FOBT, TPR.FOBT, model, .keep_all = TRUE)
  
  label_offset <- 0.05  # Adjust this to control the distance of the label from the point
  
  p <- ggplot(all_roc_data, aes(x = FPR, y = TPR, color = model, linetype = model)) +
    geom_line(size = 1.8, alpha = alpha) +
    geom_point(
      aes(x = FPR.FOBT, y = TPR.FOBT, shape = Trained_on),
      size = 2,
      stroke = 1.5, show.legend = T
    ) +
    scale_color_manual(values = setNames(colours, labels), labels = wrapped_labels) +
    scale_linetype_manual(values = setNames(linetypes, labels), labels = wrapped_labels) +
    ...
    guides(
      color = guide_legend(title = "Model", override.aes = list(
        linetype = linetypes,
        shape = NA
      )),
      linetype = "none",
      shape = guide_legend(title = "Trained on", order = 1)
    ) +
    coord_fixed() +
    geom_text(data = unique_labels,
              aes(x = FPR.FOBT + label_offset, y = TPR.FOBT, label = paste0("FPR = ", round(FPR.FOBT, 2), ", TPR = ", round(TPR.FOBT, 2))),
...
plot_comparison_scatter <- function(data, x_col, y_col, x_label, y_label, feature_column_name = "Bacteria") {
  
  # Prepare data with color coding based on criteria
  data <- data %>%
    mutate(
      point_color = case_when(
        Effect.size.16S > 0.1 & Effect.size.WGS > 0.1 & P.val.16S < 0.05 & P.val.WGS < 0.05 ~ "dodgerblue3",
        Effect.size.16S < -0.1 & Effect.size.WGS < -0.1 & P.val.16S < 0.05 & P.val.WGS < 0.05 ~ "dodgerblue3",
        TRUE ~ "white"
      ),
      enriched_in = case_when(
        (P.val.16S < 0.05 & Effect.size.16S > 0.1) | (P.val.WGS < 0.05 & Effect.size.WGS > 0.1) ~ 'CRC',
        (P.val.16S < 0.05 & Effect.size.16S < -0.1) | (P.val.WGS < 0.05 & Effect.size.WGS < -0.1) ~ 'CTR',
        TRUE ~ "n.s."
      ),
      label = ifelse(P.val.16S < 0.05 | P.val.WGS < 0.05, .data[[feature_column_name]], ""),
      font = ifelse(P.val.16S < 0.05 | P.val.WGS < 0.05, "bold.italic", "italic")
    )
...
get_siamcat_loso_metrics <- function(siamcat_list) {
  ...
  # Loop over the list of siamcat objects
  for (name in names(siamcat_list)) {
    siamcat_obj <- siamcat_list[[name]]
    
    # Extract the AUC
    auc <- siamcat_obj@eval_data[['auroc']]
    
    # Get the number of samples
    num_samples <- length(siamcat_obj@label[['label']])
...
get_siamcat_holdout_metrics<- function(siamcat_list_cv, siamcat_list_holdout){
  ...
  # Loop over the list of siamcat objects
  for (name in names(siamcat_list_cv)) {
    siamcat_obj <- siamcat_list_cv[[name]]
  ...
  for (names in names(siamcat_list_holdout)) {
    
    # Extract the AUC of holdout groups
    siamcat_test_obj = siamcat_list_holdout[[names]]
    
    for (names2 in names(siamcat_test_obj)){
      
      siamcat_train_obj = siamcat_test_obj[[names2]]
      
      # Extract the AUC
      auc <- siamcat_train_obj@eval_data[['auroc']]
      
      # Get the number of samples
      num_samples <- length(siamcat_train_obj@label[['label']])
...
get_quantile_data <- function(i, cont = "medianPredictionProb", args) {
  quantileData <- list()
  ...
  for (quantile in quantiles) {
    tmp <- i %>%
      ...
      summarize(
        value_max = quantile(.data[[cont]] + pc, probs = quantile, na.rm = TRUE),
        value_min = quantile(.data[[cont]] + pc, probs = 1 - quantile, na.rm = TRUE),
        .groups = "drop"
      ) %>%
  ...
  quantileData <- do.call(rbind, quantileData) %>%
    mutate(
      ...
      QuantileGroup = map2_chr(
        quantile, .data[[args[length(args)]]],
        ~ sprintf("%.0f%% – %.0f%%", (1 - .x) * 100, .x * 100)
      )
    ) %>%
